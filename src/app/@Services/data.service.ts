...
export class DataService {
  ...
  MapMockData() {
    ...
    this.crapData.forEach(x => {
      ...
      tran.Fee = x[3] === "TRF" ? 0.50 : 0.70;

      this.mockData.push(tran);
    });
    
    if (this.selectedAccount == null) this.selectedAccount = this.mockData[0].Account;
  ...
  GetTransactionItems(transactions: Transaction[]) {
    ...
    while (sevenDaysAgo.getDate() !== new Date().getDate()) {
      ...
      item.FeeAmount = item.Transactions.reduce((curr, next) => curr += (next.FeeType === FeeType.TRF ? 0.50 : 0.70), 0);
